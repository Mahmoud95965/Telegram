...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppWelcomeController', function ($scope, $location, MtpApiManager, ChangelogNotifyService, LayoutSwitchService, FirebaseAuthService) {
  ...
  .controller('AppLoginController', function ($scope, $location, FirebaseAuthService, toaster) {
    $scope.ui = { loading: false, mode: 'login', error: '' }
    ...
    $scope.loginWithGoogle = function(){
      $scope.ui.loading = true; $scope.ui.error = ''
      FirebaseAuthService.signInWithGoogle()
        .then(finishLogin)
        .catch(function(err){ $scope.ui.error = err && err.message || 'Google sign-in failed' })
        .finally(function(){ $scope.ui.loading = false })
    }

    $scope.loginWithEmail = function(){
      $scope.ui.loading = true; $scope.ui.error = ''
      FirebaseAuthService.signInWithEmail($scope.form.email, $scope.form.password)
        .then(finishLogin)
        .catch(function(err){ $scope.ui.error = err && err.message || 'Login failed' })
        .finally(function(){ $scope.ui.loading = false })
    }

    $scope.registerWithEmail = function(){
      $scope.ui.loading = true; $scope.ui.error = ''
      FirebaseAuthService.registerWithEmail($scope.form.email, $scope.form.password)
        .then(finishLogin)
        .catch(function(err){ $scope.ui.error = err && err.message || 'Registration failed' })
        .finally(function(){ $scope.ui.loading = false })
    }

    $scope.forgotPassword = function(){
      $scope.ui.loading = true; $scope.ui.error = ''
      FirebaseAuthService.sendPasswordReset($scope.form.email)
        .then(function(){ toaster.pop('success', 'Reset email sent', 'Check your inbox for reset instructions') })
        .catch(function(err){ $scope.ui.error = err && err.message || 'Failed to send reset email' })
        .finally(function(){ $scope.ui.loading = false })
    }
  })
